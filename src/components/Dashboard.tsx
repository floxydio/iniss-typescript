...
import axiosNew from "./axios_config";

function ResponsiveDrawer(props: any) {

  interface CheckRole {
    nama: string;
    check: (navigate: any) => Promise<void>;
    checkNavigateRole: (navigate: any) => Promise<void>;
  }

  // State
  const checkRole = useRefresh((state) => state) as CheckRole;
  ...
  // function onChangeNav(id: number) {
  //   setChangeNav(id);
  //   if (id === 2) {
  //     navigate("/guru");
  //   } else if (id === 1) {
  ...
  function onChangeNavSpesific(id: number) {
    // // console.log(`ID -> ${id}`);
    setChangeNav(id);
    if (id === 0) {
      navigate("/");
